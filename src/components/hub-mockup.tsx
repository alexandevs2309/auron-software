...
export function HubMockup({ gradient = 'linear-gradient(135deg, #1A56DB, #123F9E)' }: { gradient?: string }) {
  ...
  const navItems = [
    { icon: LayoutDashboard, label: lang === 'es' ? 'Resumen' : 'Overview' },
    { icon: Boxes, label: lang === 'es' ? 'Productos' : 'Products' },
    { icon: Briefcase, label: lang === 'es' ? 'Servicios' : 'Services' },
    { icon: Users, label: lang === 'es' ? 'Clientes' : 'Customers' },
    { icon: BarChart3, label: lang === 'es' ? 'Reportes' : 'Reports' },
  ]

  const kpis = [
    { label: lang === 'es' ? 'Cumplimiento e-CF DGII' : 'DGII e-CF compliance', value: '100%' },
    { label: lang === 'es' ? 'Operación sin internet' : 'Offline operation', value: lang === 'es' ? 'Sí' : 'Yes' },
    { label: lang === 'es' ? 'Ediciones de la suite' : 'Suite editions', value: '4' },
    { label: lang === 'es' ? 'Datos y desarrollo' : 'Data and development', value: 'RD' },
  ]
      ...
      <div className="flex min-h-[280px]">
          ...
          {navItems.map((item) => (
            <div
              ...
              className={cn(
                ...
                item.label === (lang === 'es' ? 'Resumen' : 'Overview')
                  ? 'text-white'
                  : 'text-[var(--auron-text-tertiary)]',
              )}
              style={item.label === (lang === 'es' ? 'Resumen' : 'Overview') ? { background: gradient } : undefined}
            >
        ...
        <div className="flex-1 min-w-0 bg-[var(--auron-bg)]">
          <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-[var(--auron-border-light)]">
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--auron-accent-text)' }}>
                {lang === 'es' ? 'Hub corporativo' : 'Corporate hub'}
              </div>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-xl border border-[var(--auron-card-border)] bg-[var(--auron-card-bg)] p-3">
              <div className="flex items-center justify-between mb-2.5">
                <span className="text-[10px] font-semibold text-[var(--auron-text-secondary)]">
                  {lang === 'es' ? 'Portfolio de productos' : 'Product portfolio'}
                </span>
                <span className="flex items-center gap-1 text-[9px] text-[var(--auron-text-tertiary)]">
                  <TrendingUp className="w-2.5 h-2.5" /> {lang === 'es' ? 'Un núcleo, 4 verticales' : 'One core, 4 verticals'}
                </span>
              </div>
            ...
            <div className="rounded-xl border border-[var(--auron-card-border)] bg-[var(--auron-card-bg)] p-3">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-semibold text-[var(--auron-text-secondary)]">
                  {lang === 'es' ? 'Ingresos de la compañía · 12 meses' : 'Company revenue · 12 months'}
                </span>
              </div>
